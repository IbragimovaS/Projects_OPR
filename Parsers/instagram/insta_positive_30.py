...
def get_channels():
    ...
    cursor = con.cursor()
    sql = 'select channel_id from tl_media_channels where source_id = 2 order by id limit(30) offset(90)'
...
cursor = con.cursor()
try:
    bot_message = "InstaPosivive30 Running" +  "  " +  str(datetime.datetime.now())
    ...
    bot_message = "InstaPositive30 ERROR" +  "  " +  str(datetime.datetime.now())
    telegram_bot_sendtext(bot_message)
