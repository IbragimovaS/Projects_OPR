...
def get_channels():
    ...
    cursor = con.cursor()
    sql = 'select channel_id from tl_media_channels where source_id = 2 order by id limit(30)'
...
cursor = con.cursor()
try:
    bot_message = "InstaPosivive0 Running" +  "  " +  str(datetime.datetime.now())
    ...
    bot_message = "InstaPositive0 ERROR" +  "  " +  str(datetime.datetime.now())
    telegram_bot_sendtext(bot_message)
