...
def get_channels():
    ...
    cursor = con.cursor()
    sql = 'select channel_id from tl_media_channels where source_id = 1002 order by id limit(25) offset(100)'
...
cursor = con.cursor()
try:
    bot_message = "InstaNegative40 Running" +  "  " +  str(datetime.datetime.now())
    ...
    cursor.close()
    con.close()
    bot_message = "InstaNegative40 Error " + "  " + str(datetime.datetime.now())
    telegram_bot_sendtext(bot_message)
