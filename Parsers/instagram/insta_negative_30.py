...
def get_channels():
    ...
    cursor = con.cursor()
    sql = 'select channel_id from tl_media_channels where source_id = 1002 order by id limit(20) offset(60)'
...
cursor = con.cursor()
try: 
    bot_message = "InstaNegative30 Running" +  "  " +  str(datetime.datetime.now())
    ...
    bot_message = "InstaNegative30 Error " + "  " + str(datetime.datetime.now())
    telegram_bot_sendtext(bot_message)
cursor.close()
con.close()
