...
def get_channels():
    ...
    cursor = con.cursor()
    sql = 'select channel_id from tl_media_channels where source_id = 1002 order by id limit(25) offset(50)'
...
cursor = con.cursor()
try:
    bot_message = "InstaNegative20 Running" +  "  " +  str(datetime.datetime.now())
    ...
    bot_message = "InstaNegative20 Error " + "  " + str(datetime.datetime.now())
    telegram_bot_sendtext(bot_message)
cursor.close()
con.close()
